<script type="text/javascript">
	...
	//Buku
	function tampilBuku() {
		$.get('<?php echo base_url('Buku/tampil'); ?>', function(data) {
			MyTable.fnDestroy();
			$('#data-buku').html(data);
			refresh();
		});
	...
	$(document).on("click", ".update-dataBuku", function() {
		var id = $(this).attr("data-id");
	...
	$('#form-tambah-buku').submit(function(e) {
		...
		$.ajax({
				...
				cache: false,
				timeout: 800000,
				url: '<?php echo base_url('Buku/prosesTambah'); ?>',
				data: data
			})
			.done(function(data) {
				var out = jQuery.parseJSON(data);
	...
	$(document).on('submit', '#form-update-buku', function(e) {
		event.preventDefault();

		var form = $('#form-update-buku')[0];
		...
		$.ajax({
				...
				cache: false,
				timeout: 800000,
				url: '<?php echo base_url('Buku/prosesUpdate'); ?>',
				data: data
			})
			.done(function(data) {
				var out = jQuery.parseJSON(data);
				...
				if (out.status == 'form') {
					...
					document.getElementById("form-update-buku").reset();
					$('#update-buku').modal('hide');
					$('.msg').html(out.msg);
					effect_msg();
				}
			})
	...
	$('#tambah-buku').on('hidden.bs.modal', function() {
		$('.form-msg').html('');
	})

	$('#update-buku').on('hidden.bs.modal', function() {
		$('.form-msg').html('');
	})
	...
	function tampilMember() {
		$.get('<?php echo base_url('Member/tampil'); ?>', function(data) {
			MyTable.fnDestroy();
			$('#data-member').html(data);
			refresh();
		});
	}

	var id_member;
	$(document).on("click", ".konfirmasiHapus-Member", function() {
		id_member = $(this).attr("data-id");
	})
	$(document).on("click", ".hapus-dataMember", function() {
		var id = id_member;
	...
	$(document).on("click", ".update-dataMember", function() {
		var id = $(this).attr("data-id");
	...
	$(document).on('submit', '#form-update-member', function(e) {
		...
		$.ajax({
				...
				cache: false,
				timeout: 800000,
				url: '<?php echo base_url('Member/prosesUpdate'); ?>',
				data: data
			})
			.done(function(data) {
				var out = jQuery.parseJSON(data);
				...
				if (out.status == 'form') {
					...
					document.getElementById("form-update-member").reset();
	...
	$('#update-member').on('hidden.bs.modal', function() {
		$('.form-msg').html('');
	})
</script>